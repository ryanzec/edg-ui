...
- **Duplicated pattern-documentation comment** — the comment re-documents a pattern that is already documented elsewhere (e.g. a pattern in `.cursor/rules/patterns`) instead of adding local value. The pattern docs are the source of truth; restating them inline is duplication.
